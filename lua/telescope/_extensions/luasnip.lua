--[[
Original code is form here:
https://github.com/benfowler/telescope-luasnip.nvim/blob/master/lua/telescope/_extensions/luasnip.lua
-- ]]
    ...
    if context then
        ...
        if snips_for_ft then
            for _, snippet in pairs(snips_for_ft) do
                if
                    context.name == snippet.name
                    and context.trigger == snippet.trigger
                then
...
local luasnip_fn = function(opts)
    ...
    local make_display = function(entry)
        return displayer({
            ...
            filter_description(
                entry.value.context.name,
                entry.value.context.description
            ),
        })
    end
